...
dx = L / N;
x = (0:N) .* dx; % length N + 1
y = -sin(pi * x / L) - sin(2 * pi * x / L) + sin(6 * pi * x / L);
NR_PlotXY(x,y,0,0,L,-3,3)
...
aexbt = [0, 0, 0; ...
    3375509829940 / 42525919076317, 0, 0; ...
    0, 272778623835 / 1039454778728, 0];
aimbt = [0, 0, 0; ...
    0, 3375509829940 / 4525919076317, 0;
    0, 11712383888607531889907 / 32694570495602105556248, 566138307881 / 912153721139];
...
for tStep= 1:Tmax / dt
    ...
    dydt = -y(2:N) .* (y(3:N + 1) - y(1:N-1));
    if (k == 1)
        ex_store = x; 
    else 
        ex_store = x + (aimbt(k, k - 1) - bbt(k - 1)) * dt .* im_store + ...
            (aexbt(k, k - 1) - bbt(k - 1)) * dt .* ex_store;
    end
    im_store = (eyes(4) - aimbt(k, k) * dt  .* A) \ A * ex_store;
    ex_store = ex_store
    end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END OF RK LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if (mod(tStep,PlotInterval)==0) NR_PlotXY(x,y,tStep*dt,0,L,-3,3); end
end
